...
const[filterSelected,setFilterSelected] = useState(1);
  ...
  const fetchApis = async   () => { 
    const response = await fetch("https://dev-api.wall.app/api/v1/core/campaigns/");
    ...
    <section className={styles.QuestsSection}>
      <div className='container'>
        ...
        <div className={styles.QuestsFilters}>
          <FilterButton onFilterBtnClick={()=> setFilterSelected(1)}  BtnName='All' className={filterSelected === 1 ? filterBtnStyles.BtnSelected : "" } />
          <FilterButton onFilterBtnClick={()=> setFilterSelected(2)} BtnName='On Chain' className={filterSelected === 2 ? filterBtnStyles.BtnSelected : "" } />
          <FilterButton onFilterBtnClick={()=> setFilterSelected(3)} BtnName='Social' className={filterSelected === 3 ? filterBtnStyles.BtnSelected : "" }>
            <span className={filterBtnStyles.BtnIconWrapper}><Image alt='image' src={twitterIcon} /></span>
          </FilterButton>
          <FilterButton onFilterBtnClick={()=> setFilterSelected(4)} BtnName='Token' className={filterSelected === 4 ? filterBtnStyles.BtnSelected : "" }>
            <span className={filterBtnStyles.BtnIconWrapper}><Image alt='image' src={dollorIcon} /></span>
          </FilterButton >
          <FilterSelector onChangeFilter ={(e)=>setFilterSelected(e)} selectedMenuBtn ={filterSelected} />
        </div>  
      </div>
